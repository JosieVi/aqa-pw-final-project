...
test.describe('[API] [Customers] Create A New Customer', () => {
    ...
    positiveCreateCustomerCases.forEach(({ name, data, expectedStatusCode, isSuccess, errorMessage }) => {
      test(
        ...
        async ({ workerToken, customersController, dataDisposalUtils }) => {
          const response = await customersController.create(data, workerToken);
  ...
  test.describe('Negative', () => {
    ...
    test(
      ...
      { tag: [TAGS.API, TAGS.CUSTOMERS, TAGS.REGRESSION] },
      async ({ workerToken, customersController, customersApiService, dataDisposalUtils }) => {
        const customer1Data = generateCustomerData();
        const createResponse1 = await customersApiService.createCustomer(workerToken, customer1Data);
        ...
        const createResponse2 = await customersController.create(customer2Data, workerToken);
